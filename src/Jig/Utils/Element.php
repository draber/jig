...
class Element {
  ...
  public function __call($element, $arguments=array()) {
    ...
    if(method_exists(__CLASS__, $specialMethod)) {      
      return self::$specialMethod($element_setup['attributes'],
                                     $element_setup['settings']);
    }
    else {
      return self::build($element_setup['attributes'],
                                   $element_setup['settings']);
    }
    return false;
  }
   ...
   *
   * @param string $element, tag name of the element
  ...
  protected static function prepare($attributes=array(), $settings=array()) {
    ...
    if(is_numeric(self::$settings['global settings']['indent'])) {
      self::$settings['global settings']['indent'] = str_repeat(' ', self::$settings['global settings']['indent']);
    }
  ...
  protected static function build($attributes, $settings) {
    ...
    if($content_position !== '><' && empty($attributes[$content_position])) {
  ...
  protected static function attributes($attributes, $settings) {
    ...
    foreach($attributes as $key => $value) {
      ...
      if($value === true) {
        ...
        // attibutes without a value and not allowed to be empty
