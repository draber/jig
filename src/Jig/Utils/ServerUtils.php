...
class ServerUtils {
  ...
  public static function getScriptUri($qualified = false) {
    if (!empty($_SERVER['REQUEST_URI'])) {
      return $qualified ? self::getBaseUri() . '/' . ltrim($_SERVER['REQUEST_URI'], '/') : $_SERVER['REQUEST_URI'];
    }
    if (!empty($_SERVER['REDIRECT_URL'])) {
      return $qualified ? self::getBaseUri() . '/' . ltrim($_SERVER['REDIRECT_URL'], '/') : $_SERVER['REDIRECT_URL'];
    }
    $queryString = '';
    ...
    if ($phpSelf) {
      return $qualified ? self::getBaseUri() . '/' . ltrim($phpSelf . $queryString, '/') : $phpSelf . $queryString;
    }
    return '';
  }
  ...
  public static function forceDownload($path, $dwlFilenmae = null) {
    if (!is_file($path)) {
      throw new Exception(__METHOD__ . '() File ' . $path . ' doesn\'t exist!');
    }

    if (!$dwlFilenmae) {
      $dwlFilenmae = basename($path);
    }
    ...
    header('Content-Disposition: ' . $dwlFilenmae);
    print($content);
    exit(-1);
  }
